...
export default class TicTacToeScene extends BaseScene {
	...
	async init() {
		...
		await this.assets.loadImage(
			"baltazar",
			"/pictures/tictactoeGame/baltazar.webp"
		);
    ...
    `;
		...
		this.btnGame.addEventListener("click", () => this.createGameScreen());
	...
	createUputeScreen() {
      ...
      <div class="secondLayerInstructions layer">
        ...
        <img class="imgProfBaltazar tictactoe-baltazar-img" src="${
					this.assets.images.get("baltazar").src
				}"/>
      </div>
    `;
		...
		this.btnBack = this.sceneEl.querySelector("#btnBack");
		this.btnBack.addEventListener("click", () => this.createMenuScreen());
	...
	createGameScreen() {
      ...
      <div class="secondLayerGame layer">
        ...
        <div id="currentPlayer" class="textStyle tictactoe-current-player">
          Na redu je igrač: 
          <img class="tictactoe-player-img" src="${this.getPlayerImg(
						this.currentPlayer
					)}"/>
        </div>
	...
	fillContainer() {
		const boardContainer =
			document.getElementsByClassName("tictactoe-board")[0];
