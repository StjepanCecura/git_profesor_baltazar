...
export default class StartMenuScene extends BaseScene {
	...
	async init() {
		...
		await this.assets.loadImage(
			"pamtilicaLogo",
			"/pictures/pamtilicaGame/icon.webp"
		);
		await this.assets.loadImage(
			"labyrinthLogo",
		...
		this.games = [
			...
			{
				name: "Kamen papir škare",
				logo: this.assets.images.get("KSPLogo").src,
				scene: "KSP",
			},
			{
				name: "Križić-kružić",
			...
			{
				name: "Sjekač voćki",
				logo: this.assets.images.get("sjekacVockiLogo").src,
				scene: "SjekacVocki",
			},
			{
				name: "Pamtilica",
				logo: this.assets.images.get("pamtilicaLogo").src,
				scene: "Pamtilica",
			},
		];
      ...
      <div class="firstLayer layer">
        <img class="imgProfBaltazar" src="${
					this.assets.images.get("profBaltazar").src
				}" />
      </div>
