...
export default class SjekacVockiScene extends BaseScene {
	...
	async init() {
		...
		await this.assets.loadImage(
			"cursor",
			"/pictures/sjekacVockiGame/sword1.webp"
		);
		await this.assets.loadImage(
			"cursorTip",
			"/pictures/sjekacVockiGame/sword1.webp"
		...
		await this.assets.loadImage(
			"background1",
			"/pictures/sjekacVockiGame/background1.webp"
		);
		await this.assets.loadImage(
			"background2",
			"/pictures/sjekacVockiGame/background2.webp"
		);
		...
		await this.assets.loadImage(
			"sword2",
			"/pictures/sjekacVockiGame/sword2.webp"
		);
		await this.assets.loadImage(
			"sword3",
			"/pictures/sjekacVockiGame/sword3.webp"
		);
		await this.assets.loadImage(
			"sword4",
			"/pictures/sjekacVockiGame/sword4.webp"
		);

		await this.assets.loadImage(
			"apple",
			"/pictures/sjekacVockiGame/apple.webp"
		);
		await this.assets.loadImage(
			"banana",
			"/pictures/sjekacVockiGame/banana.webp"
		);
		await this.assets.loadImage(
			"lemon",
			"/pictures/sjekacVockiGame/lemon.webp"
		);
		await this.assets.loadImage(
			"orange",
			"/pictures/sjekacVockiGame/orange.webp"
		);
		await this.assets.loadImage(
			"pineapple",
			"/pictures/sjekacVockiGame/pineapple.webp"
		);
		await this.assets.loadImage(
			"strawberry",
			"/pictures/sjekacVockiGame/strawberry.webp"
		);
		await this.assets.loadImage(
			"watermelon",
			"/pictures/sjekacVockiGame/watermelon.webp"
		);
		...
		await this.assets.loadImage(
			"appleslice1",
			"/pictures/sjekacVockiGame/appleslice1.webp"
		);
		await this.assets.loadImage(
			"appleslice2",
			"/pictures/sjekacVockiGame/appleslice2.webp"
		);
		await this.assets.loadImage(
			"bananaslice1",
			"/pictures/sjekacVockiGame/bananaslice1.webp"
		);
		await this.assets.loadImage(
			"bananaslice2",
			"/pictures/sjekacVockiGame/bananaslice2.webp"
		);
		await this.assets.loadImage(
			"lemonslice1",
			"/pictures/sjekacVockiGame/lemonslice1.webp"
		);
		await this.assets.loadImage(
			"lemonslice2",
			"/pictures/sjekacVockiGame/lemonslice2.webp"
		);
		await this.assets.loadImage(
			"orangeslice1",
			"/pictures/sjekacVockiGame/orangeslice1.webp"
		);
		await this.assets.loadImage(
			"orangeslice2",
			"/pictures/sjekacVockiGame/orangeslice2.webp"
		);
		await this.assets.loadImage(
			"pineappleslice1",
			"/pictures/sjekacVockiGame/pineappleslice1.webp"
		);
		await this.assets.loadImage(
			"pineappleslice2",
			"/pictures/sjekacVockiGame/pineappleslice2.webp"
		);
		await this.assets.loadImage(
			"strawberryslice1",
			"/pictures/sjekacVockiGame/strawberryslice1.webp"
		);
		await this.assets.loadImage(
			"strawberryslice2",
			"/pictures/sjekacVockiGame/strawberryslice2.webp"
		);
		await this.assets.loadImage(
			"watermelonslice1",
			"/pictures/sjekacVockiGame/watermelonslice1.webp"
		);
		await this.assets.loadImage(
			"watermelonslice2",
			"/pictures/sjekacVockiGame/watermelonslice2.webp"
		);
	...
	createMenuScreen() {
		...
		this.sceneEl.appendChild(
			this.createBackButton(() => this.manager.switch("StartMenu"))
		);
		...
		const btnStart = this.createButton("Nova igra", () =>
	...
	createUputeScreen() {
		...
		this.sceneEl.appendChild(
			this.createBackButton(() => this.createMenuScreen())
		);
		...
		if (this.inGame && !this.gameOver) {
			...
			this.slices.forEach((slice, index) => {
				...
				if (slice.rotationSpeed) {
					...
					const newRotation =
						currentRotation + slice.rotationSpeed * (dt / 16.67);
					slice.el.style.transform = currentTransform.replace(
						/rotate\([^)]+\)/,
	...
	sliceFruit(fruit, originalElement, originalX) {
		...
		leftSlice.src = this.assets.images.get(
			`${fruit.name}slice${isReversed ? "1" : "2"}`
		).src;
		...
		rightSlice.src = this.assets.images.get(
			`${fruit.name}slice${isReversed ? "2" : "1"}`
		).src;
	...
	checkCollisions() {
		...
		this.fruits.forEach((fruit, index) => {
			const fruitCenterX =
				parseFloat(fruit.el.style.left) + fruit.el.offsetWidth / 2;
			const fruitCenterY =
				parseFloat(fruit.el.style.top) + fruit.el.offsetHeight / 2;
		...
		this.bombs.forEach((bomb, index) => {
			const bombCenterX =
				parseFloat(bomb.el.style.left) + bomb.el.offsetWidth / 2;
			const bombCenterY =
				parseFloat(bomb.el.style.top) + bomb.el.offsetHeight / 2;
	...
	createEndScreen() {
		...
		const playAgainBtn = this.createButton("Nova igra", () =>
			this.createGameScreen()
		);

		const menuBtn = this.createButton("Izbornik", () =>
			this.createMenuScreen()
		);
