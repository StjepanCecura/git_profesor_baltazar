...
export default class InputManager {
	...
	_detectionLoop = async () => {
		...
		if (
			now - this.inputInactive > this.inactivityTimeOut &&
			!Utils.getVideoMode()
		) {
			Utils.setVideoMode();
		}
		...
		try {
			...
			this.handPredictions.forEach((pred, id) => {
				if (
					...
					this.lostHands.set(id, { ...pred, lastSeen: now });
			...
			detections.forEach((det) => {
				...
				if (bestId !== null && bestDist < 0.15) {
					...
					let reusedId = null;
					this.lostHands.forEach((lost, id) => {
						const dist = Math.hypot(lost.x - det.x, lost.y - det.y);
						if (dist < 0.9 && now - lost.lastSeen < 500) {
							reusedId = id;
						}
					});
					...
					if (reusedId !== null) {
						det.id = reusedId;
						this.lostHands.delete(reusedId);
					} else {
